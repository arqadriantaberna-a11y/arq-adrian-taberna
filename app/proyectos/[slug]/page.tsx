...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <><SiteHeader /><main className="detail-page">
      ...
      <div className="detail-hero"><Image src={project.cover} alt={project.title} fill priority sizes="100vw" /></div>
      ...
      <section className="project-flow section-shell">
        {project.images.slice(1).map((image, index) => <figure className={`${image.kind === "wide" ? "wide" : "technical"} flow-${index % 3}`} key={image.src}><div><Image src={image.src} alt={image.alt} fill sizes={image.kind === "wide" ? "100vw" : "60vw"}/></div>{image.caption && <figcaption>{image.caption}</figcaption>}</figure>)}
      </section>
